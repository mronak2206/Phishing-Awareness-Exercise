...
function selectBox(boxNumber) {
    if (selectedBox === null) {
        ...
        if (boxNumber === 1) {
            box1.classList.add('selected-green'); // Correct answer
            incrementScore(); // Increment score
            box2.classList.add('not-selected'); // Indicate unselected box
        } else {
            box2.classList.add('selected-red'); // Wrong answer
...
function goToNextPage() {
    window.location.href = 'exercise2.html'; // Change this to your next exercise page
}
